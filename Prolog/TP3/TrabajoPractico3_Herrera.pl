...
%Regla4
%Regla4 sin findall
regla4sinFA(X):-variacionRansomware(_,X,_,_,LISTA),(member('enero',LISTA);member('diciembre',LISTA)).
%Regla4 con findall
regla4(X):-findall(X,(variacionRansomware(_,X,_,_,LISTA),(member('enero',LISTA);member('diciembre',LISTA))),X).
%Regla5
